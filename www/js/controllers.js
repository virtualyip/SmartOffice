...
.controller('AnnyangController', function($scope) {
  ...
  $scope.init = function() {
    ...
    $scope.Recognition.callback = function (err, result) {
      ...
      $scope.listening = false;
      if($scope.speech_actions[result.transcript]){
        $scope.speech_actions[result.transcript]();
      }
    }
    ...
    $scope.speech_actions = window.localStorage.getItem("speech_actions");
    if(!$scope.speech_actions){
      $scope.speech_actions = $scope.default_action;
    }
  }
